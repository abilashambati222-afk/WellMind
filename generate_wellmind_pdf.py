...
class RTRPPDF(FPDF):
    def __init__(self):
        ...
        self.doc_title = "WELLMIND - AI ASSISTED MENTAL HEALTH SUPPORT PORTAL"
        self.dept_text = "Department of Computer Science and Engineering, KMIT"
...
def generate_pdf():
    md_path = "WellMind_KMIT_RTRP_Documentation.md"
    ...
    while idx < len(lines):
        ...
        if line.startswith("## Cover Page") or line.startswith("## Bonafide Certificate") or \
           ...
           line.startswith("## Chapter") or line.startswith("## Course Outcome"):
            
            # Start normal numbering after front matter
        ...
        if line.startswith("|") and idx + 1 < len(lines) and lines[idx+1].strip().startswith("| :---"):
            ...
            # Column widths heuristic for WellMind mapping tables
            if col_count == 13: # CO-PO Mapping Table
                col_widths = [w_total * 0.16] + [w_total * 0.07] * 12
            elif col_count == 2:
                col_widths = [w_total * 0.4, w_total * 0.6]
            elif col_count == 3:
                col_widths = [w_total * 0.25, w_total * 0.5, w_total * 0.25]
            elif col_count == 4:
                col_widths = [w_total * 0.2, w_total * 0.25, w_total * 0.25, w_total * 0.3]
            elif col_count == 5:
                col_widths = [w_total * 0.08, w_total * 0.35, w_total * 0.2, w_total * 0.27, w_total * 0.1]
            elif col_count == 7: # Survey table
                col_widths = [w_total * 0.16, w_total * 0.14, w_total * 0.14, w_total * 0.14, w_total * 0.14, w_total * 0.14, w_total * 0.14]
            else:
        ...
        if line:
            text = clean_txt(line)
            align_val = "L"
            if "DEPARTMENT OF" in text or "KESHAV MEMORIAL" in text or "A Project Report" in text or "Report on" in text or "BACHELOR OF" in text:
                align_val = "C"
                pdf.set_font("Times", "B", 12)
            elif "Submitted by" in text or "Under the Guidance of" in text or "BONAFIDE CERTIFICATE" in text or "DECLARATION" in text:
                align_val = "C"
                pdf.set_font("Times", "B", 12)
            else:
                pdf.set_font("Times", "", 12)
    ...
    output_pdf_path = "WellMind_KMIT_RTRP_Documentation.pdf"
